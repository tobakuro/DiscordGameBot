    ...
    async def on_submit(self, it: discord.Interaction):
        ...
        if not val.isdigit():
            await it.response.send_message(
                "数字のみを入力してください。", ephemeral=True
            )
            return

        await it.response.send_message(
            f"✅ 回答 `{val}` を送信しました。全員の完了を待っています...",
            ephemeral=True,
        )
        # 親ポータルに回答を通知
...
class AnswerPortalView(discord.ui.View):
    ...
    @discord.ui.button(
        label="回答を入力する", style=discord.ButtonStyle.primary, emoji="⌨️"
    )
...
class SetupAndJoinView(discord.ui.View):
    ...
    def make_embed(self):
        if not self.is_recruiting:
            ...
            embed.add_field(
                name="個数", value=f"**{self.count}** 個\n(※3個以上)", inline=True
            )
            embed.add_field(
                name="速度", value=f"**{self.speed:.1f}** 秒間隔", inline=True
            )
            return embed
        else:
            names = (
                "\n".join([f"・{u.display_name}" for u in self.participants])
                or "待機中..."
            )
            ...
            embed.set_footer(
                text=f"難易度: {self.digits}桁 / {self.count}個(3個〜) / {self.speed}s"
            )
            return embed

    @discord.ui.button(label="桁数＋", row=0)
    async def d_up(self, it, b):
        self.digits = min(5, self.digits + 1)
        await it.response.edit_message(embed=self.make_embed())

    @discord.ui.button(label="桁数－", row=0)
    async def d_down(self, it, b):
        self.digits = max(1, self.digits - 1)
        await it.response.edit_message(embed=self.make_embed())

    @discord.ui.button(label="個数＋", row=1)
    async def c_up(self, it, b):
    ...
    async def start_recruit(self, it, b):
        ...
        btn = discord.ui.Button(
            label="参加する！", style=discord.ButtonStyle.primary, emoji="🙋"
        )
...
@app_commands.command(
    name="flash", description="テキストフォーム入力式のフラッシュ暗算対戦"
)
async def flash(interaction: discord.Interaction):
    ...
    numbers = [
        random.randint(10 ** (view.digits - 1), (10**view.digits) - 1)
    ...
    await msg.edit(
        content="⌛ **全員回答してください！ 下のボタンから入力できます。**",
        view=portal,
    )
    ...
    # リザルト & API保存
    from .modules import flash_result_correct_request, flash_result_play_request
    ...
    for user in view.participants:
        ...
        # play_count保存
        flash_result_play_request(
            token=interaction.client.token,
            discord_id=user.id,
            username=user.display_name,
        )
        # correct_count保存（正解者のみ）
        if is_correct:
            flash_result_correct_request(
                token=interaction.client.token,
                discord_id=user.id,
                username=user.display_name,
            )
